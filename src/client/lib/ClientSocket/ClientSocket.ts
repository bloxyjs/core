...
export declare interface Socket extends EventEmitter {
  on(
    ...
    listener: (data: { conversation: BaseChatConversation }) => void
  ): this;
  ...
  on(
    ...
    listener: (data: {
      user: BaseUser;
      conversation: BaseChatConversation;
    }) => void
  ): this;
  ...
  on(event: "friendLost", listener: (data: { user: BaseUser }) => void): this;

  on(
    event: "friendRequest",
    listener: (data: { user: BaseUser }) => void
  ): this;

  on(event: "friendAdded", listener: (data: { user: BaseUser }) => void): this;
    ...
    return new Promise((resolve, reject) => {
      const connectSocket = (retries = 0): void => {
        this.socket = new SignalR.client(
          "wss://realtime.roblox.com/notifications",
          ["usernotificationhub"],
          3,
          true
        );
        ...
        if (retries >= maxRetries) {
          ...
          reject(
            new Error(
              `Connection failed, attempted to establish a connection ${retries} times`
            )
          );
        }

        this.socket.serviceHandlers.connectFailed = (error: Error): unknown =>
          onError(error);
        this.socket.serviceHandlers.onerror = (error: Error): unknown =>
          onError(error);
